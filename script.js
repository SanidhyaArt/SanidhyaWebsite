...
if (hiddenPage && window.sessionStorage.getItem(freebiesAccessKey) !== "true") {
  window.location.replace("./freebies.html");
}
...
if (
  ...
  const workFeatureCards = workTitleItems.map((item, index) => {
    ...
    card.href = item.dataset.href || "./work.html";
  ...
  workScrollHost.addEventListener(
    ...
    (event) => {
      ...
      const nextScrollTop = workTitleRail.scrollTop + event.deltaY;
      const maxScrollTop = workTitleRail.scrollHeight - workTitleRail.clientHeight;
      const canScrollDown = event.deltaY > 0 && workTitleRail.scrollTop < maxScrollTop - 1;
      const canScrollUp = event.deltaY < 0 && workTitleRail.scrollTop > 1;
